...
public class UploadDocumentEndpointTests
{
    ...
    private readonly UploadDocumentEndpoint _endpoint;

    public UploadDocumentEndpointTests()
    {
        _publishEndpointMock = new Mock<IPublishEndpoint>();
        _fileStorageServiceMock = new Mock<IFileStorageService>();
        _endpoint = new UploadDocumentEndpoint(_publishEndpointMock.Object, _fileStorageServiceMock.Object);
    }
    ...
    [Fact]
    public async Task UploadDocument_ShouldReturn400_WhenFileIsNull()
    {
        // Arrange
        ...
        _publishEndpointMock.Verify(x => x.Publish(It.IsAny<ConvertDocumentToJsonCommand>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Fact]
    public async Task UploadDocument_ShouldReturn400_WhenFileExtensionIsInvalid()
    {
        // Arrange
        var mockFile = CreateMockFormFile("malicious.exe", Encoding.UTF8.GetBytes("MZ_test_content"));
        var request = new UploadDocumentRequest { File = mockFile };
    ...
    [Fact]
    public async Task UploadDocument_ShouldReturn400_WhenMagicBytesValidationFails()
    {
        // Arrange
    ...
    [Fact]
    public async Task UploadDocument_ShouldReturn202_WhenFileIsValidAndMagicBytesMatch()
    {
        // Arrange
        ...
        _publishEndpointMock.Verify(x => x.Publish(It.Is<ConvertDocumentToJsonCommand>(c => c.FileName == "valid_heritage_doc.pdf" && c.FileType == "pdf"), It.IsAny<CancellationToken>()), Times.Once);
    }
}
