...
public class LocationDocumentProcessedConsumerTests
{
    ...
    public async Task Consume_ShouldCreateContributionAndBroadcastSignalR_WhenSuccessful()
    {
        ...
        // ID is generated by CreateCommunityArticle, but we mock GetByIdAsync below. 
        // We need to set the Id to match mongoId via reflection or just use the generated one for the mock.
        // Actually since we mock `GetByIdAsync(mongoId)`, the returned document's Id doesn't strictly matter for the test, 
        // but if it does, we can't set it easily now. Let's just return the created document.
